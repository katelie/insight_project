...
def GetComments_SK(soup, comments_total):
    ...
    for comment_ID, _ in enumerate(range(0, number_of_comments)):
        ...
        comments_data[comments_total+comment_ID] = GetCommentData_SK(comment, comment_ID, comment_has_children, child_id)
...
for recipenumber, url in enumerate(urls_temp): 
...
CommentsFrame = pd.DataFrame.from_dict(comments_all).T
RecipeFrame = pd.DataFrame.from_dict(recipes_all).T
...
CommentsFrame.to_csv('SmittenKitchen_Comments.csv', sep=',', encoding='utf-8')
RecipeFrame.to_csv('SmittenKitchen_Recipes.csv', sep=',', encoding='utf-8')timestamp_raw = GetText(soup.find('span',{'class':'date'})) +', '+ GetText(soup.find('span',{'class':'time'}))
timestamp = datetime.datetime.strptime(timestamp_raw, '%B %d, %Y, %I:%M %p')
timestampGetComments_SK(soup)soup.find('span',{'class':'date'}).get_text() +', '+ soup.find('span',{'class':'time'}).get_text()comments = soup.find('ol',{'class':'comment-list'})
comments_data = {}
comment_has_children, child_id = 'no', 0 
...
for comment_ID, _ in enumerate(range(0, 20)):
    print(comment_ID)
